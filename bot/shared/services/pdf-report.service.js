...
class PDFReportService {
  ...
  static _headerLabels(reportData) {
    ...
    return {
      ...
      sub:     'Teacher Performance Evaluation powered by NIETE',
    };
  }
  ...
  static _drawPriorFeedback(doc, priorFeedback, yPos) {
    ...
    if (isFirstObservation) {
      const emptyMessage = "This criterion is not applicable because this is the teacher's first observed lesson with NIETE. Future reports will assess how the teacher incorporates feedback from this observation.";
  ...
  static _drawPageFooters(doc, reportData) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      doc.fontSize(7).fillColor(this.COLORS.secondary).font('Helvetica')
         .text(`NIETE · Page ${i + 1} of ${total}`, 350, y, { width: 195, align: 'right', lineBreak: false });
